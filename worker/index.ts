...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    switch (request.method) {
      case 'GET':
        return Response.json(await readBlob(env))
      case 'PUT':
...
async function readBlob(env: Env): Promise<{ json: string | null; updatedAt: number }> {
  ...
  // "nothing to merge" and uploads its local database as the first blob.
  return row ?? { json: null, updatedAt: 0 }
}
